                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table mb-0">
                                <tbody>
                                    @php
                                    $sortedTeachers = collect($allTeachers)->sort(function ($a, $b) {
                                    ...
                                    return $b->approval <=> $a->approval;
                                        ...
                                        return $b->status <=> $a->status;
                                            ...
                                            return $a->id <=> $b->id;
                                                ...
                                                @endphp
                                                <tr>
                                                    <td><img src="{{ (!empty($user->photo)) ? url('uploads/profileimages/'.$user->photo) :  url('/noprofile/no-profile.jpg') }}" alt="Profile Image" id="" class="rounded-circle me-4 bg-primary" width="150">{{$teacher->fullname}}</td>
                                                    <td style="vertical-align: middle; text-align: start">
                                                        <a class="btn btn-outline-primary me-2" data-bs-toggle="modal" data-bs-target="{{'#details'.$teacher->id}}">More Details</a>
                                                        @if($teacher->approval == 'pending')
                                                        <a href="{{route('admin.teacher.approve',$teacher->id)}}" class="btn btn-success me-2">Approve</a>
                                                        <a href="{{route('admin.teacher.reject',$teacher->id)}}" class="btn btn-danger px-4">Reject</a>
                                                        @elseif($teacher->approval == 'active')
                                                        <button class="btn btn-success" disabled>Approved</button>
                                                        @if($teacher->status == 'active')
                                                        <a href="{{route('admin.teacher.status',$teacher->id)}}" class="btn btn-primary px-4">Active</a>
                                                        @else
                                                        <a href="{{route('admin.teacher.status',$teacher->id)}}" class="btn btn-danger px-4">De-Active</a>
                                                        @endif
                                                        @else
                                                        <button class="btn btn-danger" disabled>Rejected</button>
                                                        <a href="{{route('admin.teacher.reapprove',$teacher->id)}}" class="btn btn-warning px-4">Re-Approve</a>
                                                        @endif
                                                        @if($user->status == '1')
                                                        <a href="{{route('admin.teacher.user.suspend',$user->id)}}" class="btn btn-warning px-4">Suspend</a>
                                                        @else
                                                        <a href="{{route('admin.teacher.user.suspend',$user->id)}}" class="btn btn-danger px-4">Un-Suspend</a>
                                                        @endif
                                                    </td>
                                                </tr>
                                                @endforeach
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body d-flex justify-content-between flex-wrap px-5">
                <div class="table-responsive " style=" vertical-align: middle; text-align: start;">
                    <table class=" mb-0">
                        <tbody class="col-12 d-flex gap-2 flex-column align-items-start">
                            @php
                            $user = User::where('id',$teacher->role_id)->first();
